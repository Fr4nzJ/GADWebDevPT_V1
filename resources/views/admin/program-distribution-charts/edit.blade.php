...
@section('title', 'Edit Program Distribution Chart Data - GAD Admin Panel')

@section('content')
<!-- ===== PAGE HEADER ===== -->
<div class="page-header">
    <h1 class="page-title">Edit Program Distribution Chart Data Point</h1>
</div>

<div class="card" style="background: white; border-radius: 12px; padding: 2rem; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.08); color: #333;">
    @if ($errors->any())
    ...
    <form action="{{ route('admin.program-distribution-charts.update', $programDistributionChart->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="field">
            ...
            <div class="control">
                <input class="input" type="text" name="label" placeholder="e.g., Gender Empowerment" value="{{ $programDistributionChart->label }}" required>
            </div>
            @error('label')
        ...
        <div class="field">
            ...
            <div class="control">
                <input class="input" type="number" name="value" min="0" value="{{ $programDistributionChart->value }}" required>
            </div>
            @error('value')
                <p class="help is-danger">{{ $message }}</p>
            @enderror
            ...
            <div class="field is-horizontal">
                <div class="field-body">
                    <div class="field is-narrow" style="flex-grow: 0;">
                        <div class="control">
                            <input type="color" id="color_hex" name="color_hex" class="input" style="width: 80px; height: 50px; cursor: pointer;" value="{{ $programDistributionChart->color_hex ?? '#667eea' }}">
                        </div>
                    </div>
                    <div class="field is-expanded">
                        <div class="control has-icons-left">
                            <input class="input" type="text" name="color_hex_text" placeholder="#667eea" value="{{ $programDistributionChart->color_hex ?? '#667eea' }}" readonly>
        ...
        <div class="field">
            ...
            <div class="control">
                <input class="input" type="number" name="order" min="1" value="{{ $programDistributionChart->order }}" required>
            </div>
        ...
        <div class="field">
            <div class="control">
                <label class="checkbox">
                    <input type="checkbox" name="is_active" value="1" {{ $programDistributionChart->is_active ? 'checked' : '' }}>
