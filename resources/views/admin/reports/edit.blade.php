...
@section('title', 'Edit Report - GAD Admin Panel')
        ...
        <div class="columns is-multiline">
            <div class="column is-12">
                <div class="field">
                    ...
                    <div class="control">
                        <input class="input @error('title') is-danger @enderror" type="text" name="title" placeholder="Enter report title" value="{{ old('title', $report->title) }}" required>
                    </div>
                    @error('title')
            ...
            <div class="column is-6">
                <div class="field">
                    ...
                    <div class="control">
                        <input class="input @error('year') is-danger @enderror" type="number" name="year" min="2000" max="{{ date('Y') }}" value="{{ old('year', $report->year) }}" required>
                    </div>
                    @error('year')
                        <p class="help is-danger">{{ $message }}</p>
                    @enderror
                    ...
                    <div class="control">
                        <div class="select is-fullwidth @error('type') is-danger @enderror">
                            <select name="type" required>
                                ...
                                <option value="Survey" {{ old('type', $report->type) === 'Survey' ? 'selected' : '' }}>Survey</option>
                                <option value="Analysis" {{ old('type', $report->type) === 'Analysis' ? 'selected' : '' }}>Analysis</option>
                                <option value="Research Study" {{ old('type', $report->type) === 'Research Study' ? 'selected' : '' }}>Research Study</option>
                                <option value="Assessment" {{ old('type', $report->type) === 'Assessment' ? 'selected' : '' }}>Assessment</option>
                                <option value="Baseline Study" {{ old('type', $report->type) === 'Baseline Study' ? 'selected' : '' }}>Baseline Study</option>
                                <option value="Audit" {{ old('type', $report->type) === 'Audit' ? 'selected' : '' }}>Audit</option>
                                <option value="Budget Analysis" {{ old('type', $report->type) === 'Budget Analysis' ? 'selected' : '' }}>Budget Analysis</option>
                                <option value="Health Study" {{ old('type', $report->type) === 'Health Study' ? 'selected' : '' }}>Health Study</option>
                                <option value="Impact Study" {{ old('type', $report->type) === 'Impact Study' ? 'selected' : '' }}>Impact Study</option>
                            </select>
                        </div>
                    </div>
                    @error('type')
            ...
            <div class="column is-12">
                <div class="field">
                    ...
                    <div class="control">
                        <textarea class="textarea @error('description') is-danger @enderror" name="description" placeholder="Enter report description" rows="6" required>{{ old('description', $report->description) }}</textarea>
                    </div>
                    @error('description')
                        <p class="help is-danger">{{ $message }}</p>
                    @enderror
                ...
                <div class="field">
                    ...
                    @if($report->file_path)
                        <p style="margin-top: 0.5rem; color: #999; font-size: 0.9rem;">
                            Current file: <strong>{{ basename($report->file_path) }}</strong>
                        </p>
                    @endif
                    @error('file_path')
                        <p class="help is-danger">{{ $message }}</p>
...
<script>
    document.querySelector('.file-input').addEventListener('change', function(e) {
        const fileName = e.target.files[0]?.name || (document.getElementById('fileName').textContent || 'No file selected');
